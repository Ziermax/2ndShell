...
/*  Updated: 2025/06/19 20:32:22 by mvelazqu           ###   ########.fr      */
/*                                                                            */
/* ************************************************************************** */
...
static char	**find_matched_files(char **rules, char **files)
{
	...
	while (files[++i])
	{
		...
		if (!tmp)
			return (free_array(matches), NULL);
...
static char	**get_file_in_dir(char *dir_name)
{
	...
	while (entity)
	{
		...
		if (!tmp)
			return (free_array(dir_files), closedir(directory), NULL);
...
char	**expand_asterisk(char *string)
{
	...
	if (!rules)
		return (free_array(files), NULL);
	rules = clean_rules(rules);
	if (!rules)
		return (free_array(files), NULL);
	matched_files = find_matched_files(rules, files);
	return (free_array(files), free_array(rules), matched_files);
}
